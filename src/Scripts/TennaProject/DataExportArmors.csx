...
void TennaExportArmors()
{
  if (!TennaExportArmors(TennaSelectExportDir()))
    throw new Exception("DataExportArmors.csx failed.");
}

bool TennaExportArmors(string exportDir)
{
  return TennaExportExactNameTable("armor", "DataExportArmors.csx", "armors.json", "ARMOR", "scr_armorinfo", "case", "armornametemp", exportDir);
}
