...
void TennaExportWeapons()
{
  if (!TennaExportWeapons(TennaSelectExportDir()))
    throw new Exception("DataExportWeapons.csx failed.");
}

bool TennaExportWeapons(string exportDir)
{
  return TennaExportExactNameTable("weapon", "DataExportWeapons.csx", "weapons.json", "WEAPON", "scr_weaponinfo", "case", "weaponnametemp", exportDir);
}
