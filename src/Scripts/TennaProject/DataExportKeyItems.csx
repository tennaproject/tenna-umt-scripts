...
void TennaExportKeyItems()
{
  if (!TennaExportKeyItems(TennaSelectExportDir()))
    throw new Exception("DataExportKeyItems.csx failed.");
}

bool TennaExportKeyItems(string exportDir)
{
  return TennaExportExactNameTable("key item", "DataExportKeyItems.csx", "key-items.json", "KEY_ITEM", "scr_keyiteminfo", "case", "tempkeyitemname", exportDir);
}
