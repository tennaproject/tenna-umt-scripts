...
void TennaExportSpells()
{
  if (!TennaExportSpells(TennaSelectExportDir()))
    throw new Exception("DataExportSpells.csx failed.");
}

bool TennaExportSpells(string exportDir)
{
  return TennaExportExactNameTable("spell", "DataExportSpells.csx", "spells.json", "SPELL", "scr_spellinfo", "case", "spellname", exportDir);
}
